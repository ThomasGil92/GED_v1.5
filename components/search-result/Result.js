...
import useLocalFolder from "../../utils/useLocalFolders";
...
const Result = ({
  ...
  const [folders, setFolders] = useLocalFolder();
  ...
  const deleteFolder = (idFolder) => (e) => {
    ...
    let newFoldersResults = [];
    foldersResult.forEach((f) => {
      if (f._id !== idFolder) {
        newFoldersResults.push(f);
      }
    });
    /* UPDATE THE FOLDERS RESULT */
    setFoldersResult(newFoldersResults);
  };

  const deleteFile = (idFile, folder) => (e) => {
    e.preventDefault();
    let files = [];
    ...
    /* UPDATE THE FILES RESULT */
    let newFilesResults = [];
    filesResult.forEach((f) => {
      if (f.file._id !== idFile) {
        newFilesResults.push(f);
      }
    });
    setFilesResult(newFilesResults);
    /* SAVE THE UPDATED FOLDERS IN LOCALSTORAGE */
    localStorage.setItem("folders", JSON.stringify(foldersInLocal));
  };

  return (
    <div className="col-10 offset-2 h-100 pt-5 mt-5">
      {foldersResult && foldersResult.length !== 0 && (
            ...
            return (
              ...
              >
                <div className="col-6 pl-0 d-flex">
                  ...
                  <Link href={`/folder/${folder.name.toLowerCase()}`}>
                    {folder.name}
                  </Link>
                </div>
                <div className="col-2 font-weight-light">
                  {folder.lastModification}
                </div>
                <div className="col-2 font-weight-light">{folder.author}</div>
                <div className="col-2 font-weight-light">
      ...
      {filesResult && filesResult.length !== 0 && (
            ...
            filesResult.map((f, i) => {
              return (
                ...
                >
                  ...
                  <div className="col-2 font-weight-light">
                    {f.file.lastModification}
                  </div>
                  <div className="col-2 font-weight-light">{f.file.author}</div>
                  <div className="col-2 font-weight-light">
                    <div className="dropdown ">
                      <button
                        className="btn btn-default dropdown-toggle"
                      ...
                      >
                        <li>
                          <a
                            href="#"
                            className="d-flex dropdown-item"
                            onClick={deleteFile(f.file._id, f.folder)}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="20"
